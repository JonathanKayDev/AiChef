...
namespace AiChef.Server.Services
{
    public class OpenAIService : IOpenAIAPI
    {
        ...
        // build the function objects, so AI willa return JSON formatted objects
        ...
        public async Task<List<RecipeIdea>> CreateRecipeIdeas(string mealtime, List<string> ingredientList)
        {
            ...
            ChatFunctionResponse? functionResponse = chatResponse?.Choices?
                                                    .FirstOrDefault(c => c.Message?.FunctionCall is not null)?
                                                    .Message?
                                                    .FunctionCall;
